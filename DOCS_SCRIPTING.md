...
### 4.5 Headless WebView — `RoCatPage` (Tahap 23, Dual-Mode Scraping)

`RoCatPage` adalah **Puppeteer-like headless browser** yang memakai satu instance
`WebView` tersembunyi (tidak pernah di-attach ke hierarki view — anti-leak). Skrip
dapat memadukan `fetch()` + `RoCatDOM` (Mode Statis) dengan automasi WebView hidup
(Mode Interaktif) dalam satu alur eksekusi.

**Kontrak threading:** semua method **sinkron untuk skrip**. Engine mengeksekusi
skrip di background coroutine, jadi bridge mem-park thread itu dengan `CountDownLatch`
sambil meloncat ke main thread (WebView wajib di main thread) lewat `Handler`. UI thread
tidak pernah terblokir. Kerugian: `RoCatPage.*` HANYA boleh dipanggil dari skrip yang
dijalankan engine (bukan dari `main()` PlayGround biasa di main thread).

**Kapan pakai?** Pakai `fetch()` + `RoCatDOM` (Mode Statis) untuk 95% halaman — itu
cepat, hemat baterai, dan tidak bergantung pada engine WebView. Pakai `RoCatPage`
(Mode Interaktif) hanya saat target **benar-benar butuh browser hidup**:

| Situasi | Mode yang dipakai |
|---------|-------------------|
| HTML server-rendered (card, daftar, detail) | Statis (`fetch` + `RoCatDOM`) |
| JSON API publik | Statis (`RoCat.fetchJson`) |
| Halaman yang di-render JS (SPA, infinite-scroll) | **Interaktif** (`RoCatPage.open` lalu `getHtml()`) |
| Form login / anti-bot (Cloudflare, Turnstile) | **Interaktif** (`open` → `type` → `click` → `waitForSelector`) |
| Player iframe yang URL-nya di-generate JS | **Interaktif** (`open` → `evaluate("…")`) |
| File besar / halaman berat | Statis (WebView jauh lebih boros memori) |

**Tanda `RoCatPage` TERSEDIA di scope:** ketika host (aplikasi) menyuplai
`ScriptBrowserBridge` ke `ScriptEnvironment`, engine meng-inject `RoCatPage` ke scope.
Skrip yang butuh mengecek ketersediaan:

```javascript
if (typeof RoCatPage === "undefined") {
    RoCatUI.addAlert("Browser headless tidak tersedia di host ini.", "warning");
    return;
}
```

#### `RoCatPage.open(url, timeoutMs)`
Membuka `url` di WebView tersembunyi dan **menunggu** hingga `onPageFinished` (atau
error pemuatan) fires — atau `timeoutMs` (default **15 000 ms**) lewat.

| Return | Keterangan |
|--------|-----------|
| `true` | Halaman selesai dimuat (atau load error) dalam timeout. |
| `false` | Timeout / WebView tidak tersedia. |

```javascript
if (!RoCatPage.open("https://example.com/login", 20000)) {
    RoCatUI.addAlert("Timeout membuka halaman.", "error"); return;
}
```

#### `RoCatPage.type(selector, text)`
Mengisi elemen pertama yang cocok `selector` (CSS) dengan `text`. Mendispatch event
`focus` → `input` → `change` → `blur` agar framework reaktif (React/Vue/Svelte)
mengambil nilainya — bukan sekadar `el.value = …`.

| Return | Keterangan |
|--------|-----------|
| `true` | Elemen ditemukan & diisi. |
| `false` | Elemen tidak ada / browser gagal. |

```javascript
RoCatPage.type("input[name=username]", "admin");
RoCatPage.type("input[name=password]", "12345");
```

#### `RoCatPage.click(selector)`
Men-dispatch pointer/mouse click lengkap (`pointerdown` → `mousedown` → `pointerup` →
`mouseup` → `click`) pada elemen pertama yang cocok `selector`. Fallback ke
`el.click()` polos bila `MouseEvent` tidak tersedia.

| Return | Keterangan |
|--------|-----------|
| `true` | Elemen ditemukan & diklik. |
| `false` | Elemen tidak ada / browser gagal. |

```javascript
RoCatPage.click("button[type=submit]");
RoCatPage.waitForSelector(".dashboard", 5000); // tunggu navigasi/re-render
```

#### `RoCatPage.waitForSelector(selector, timeoutMs)`
Polling DOM yang sudah di-render (cek `document.querySelector(...)`) setiap 150 ms
hingga elemen muncul atau `timeoutMs` (default **15 000 ms**) lewat. Penting setelah
`click()` yang memicu navigasi async / render JS.

| Return | Keterangan |
|--------|-----------|
| `true` | Selector muncul dalam timeout. |
| `false` | Timeout / browser gagal. |

#### `RoCatPage.evaluate(jsScript)`
Menjalankan `jsScript` di **konteks halaman hidup** (bukan di scope Rhino) dan
mengembalikan nilai native JS. Bridge mendekode callback JSON-encoded WebView, lalu
`RoCatPageBridge` mem-parse-nya balik menjadi nilai Rhino (string / number / object /
array). Literal `undefined` di-petakan ke `null`. Return `"null"` bila WebView tak
tersedia.

```javascript
var title = RoCatPage.evaluate("document.title");                 // string
var urls  = RoCatPage.evaluate("Array.from(document.querySelectorAll('a')).map(a => a.href)");
var blob  = RoCatPage.evaluate("(function(){ var s = document.querySelector('script#player'); return s ? s.textContent : ''; })()");
```

> Setiap panggilan ke `evaluate` membuat satu WebView → JS round-trip. Untuk ekstraksi
> berat (looping semua `<script>`) lakukan dalam satu ekspresi `evaluate` agar murah.

#### `RoCatPage.getHtml()`
Mengembalikan `document.documentElement.outerHTML` — HTML yang **sudah di-render**
oleh WebView (termasuk yang di-inject JS). Cocok untuk halaman SPA yang HTML statis
nya kosong sebelum JS jalan. Output sudah di-decode dari format JSON-encoded WebView.

```javascript
RoCatPage.open("https://spa.example.com/list", 20000);
RoCatPage.waitForSelector(".item-card", 5000);
var rendered = RoCatPage.getHtml();
var doc = RoCatDOM.parse(rendered);
RoCatUI.addGrid(3, JSON.stringify(parseCards(doc)), "openDetail");
```

#### `RoCatPage.close()`
Merilis WebView tersembunyi (`stopLoading` + `removeAllViews` + `destroy`). Selalu
aman dipanggil berulang (no-op bila sudah ditutup). **Disarankan memanggil di blok
`finally`** setiap pola Mode Interaktif agar memori WebView bebas.
...
#### Contoh lengkap: hybrid Statis → Interaktif (fallback)

```javascript
/**
 * Buka halaman video: coba Mode Statis lebih dulu (fetch + Jsoup). Hanya jika
 * player URL tidak ditemukan di script html5player, naik ke Mode Interaktif
 * untuk mengekstrak via WebView hidup.
 */
function playEpisode(payload) {
    ...
    RoCat.render([
        { type: "clear" },
        { type: "button", label: "← Kembali", fn: "onLaunch" },
        { type: "alert", message: "Mencari stream untuk " + ep.title, level: "info" }
    ]);

    // --- Mode Statis (default, ringan) ---
    var res = fetch(ep.url, "GET", {}, null);
    var playUrl = "";
    var label = "";
    if (res.ok) {
        var doc = RoCatDOM.parse(res.text());
        var scripts = doc.find("script:containsData(html5player)");
        if (scripts.length > 0) {
            var src = scripts[0].innerHtml;
            playUrl = extractVideoUrl(src, "setVideoHLS");
            if (playUrl) label = "HLS (Statis)";
        }
    }

    // --- Mode Interaktif (fallback, headless WebView) ---
    if (!playUrl && typeof RoCatPage !== "undefined") {
        RoCatUI.addAlert("Mode Statis kosong — naik ke Mode Interaktif.", "warning");
        try {
            if (RoCatPage.open(ep.url, 20000)) {
                RoCatPage.waitForSelector("script", 8000);
                var data = RoCatPage.evaluate(
                    "(function(){" +
                    "  var s = document.querySelectorAll('script');" +
                    "  for (var i=0;i<s.length;i++){" +
                    "    var t = s[i].textContent || '';" +
                    "    var m = t.match(/setVideoHLS\\(['\"]([^'\"]+)['\"]\\)/);" +
                    "    if (m) return m[1];" +
                    "  }" +
                    "  return '';" +
                    "})()"
                );
                if (data) { playUrl = data; label = "HLS (Interaktif)"; }
            }
        } finally {
            RoCatPage.close();
        }
    ...
    if (playUrl) {
        var isHls = playUrl.indexOf(".m3u8") !== -1 || playUrl.indexOf("hls://") === 0;
        RoCatUI.addVideo(playUrl, ep.title + " · " + label, isHls, true);
    } else {
        RoCatUI.addAlert("Stream tidak ditemukan di kedua mode.", "error");
    }
}
```
...
// ---▶ Dipanggil saat episode diketuk — Mode Statis + fallback Mode Interaktif ---
function playEpisode(payload) {
    try {
        var ep = RoCat.safeParseJson(payload, {});
        if (!ep || !ep.url) { RoCatUI.addAlert("Episode tidak valid.", "error"); return; }
        RoCatUI.clear();
        RoCatUI.addButton("🏠 Home", "onLaunch");
        RoCatUI.log("⏳ Mencari stream: " + ep.title);

        // --- Mode Statis (default, murah) ---
        var playUrl = "";
        var label = "";
        var res = fetch(ep.url, "GET", {}, null);
        if (res.ok) {
            ...
            var scripts = doc.find("script:containsData(html5player)");
            if (scripts.length > 0) {
                var src = scripts[0].innerHtml;                       // CDATA: text() = "", innerHtml berisi JS
                playUrl = extractVideoUrl(src, "setVideoHLS") || extractVideoUrl(src, "setVideoUrlHigh");
                if (playUrl) label = "Statis";
            }
        }

        // --- Mode Interaktif (fallback, headless WebView) ---
        if (!playUrl && typeof RoCatPage !== "undefined") {
            RoCatUI.addAlert("Statis kosong — naik ke Mode Interaktif.", "warning");
            try {
                if (RoCatPage.open(ep.url, 20000)) {
                    RoCatPage.waitForSelector("script", 8000);
                    var data = RoCatPage.evaluate(
                    ...
                    if (data) { playUrl = data; label = "Interaktif"; }
                }
            } finally {
                RoCatPage.close();                                     // bebas WebView di finally
            }
        }
...
/** Ambil URL dari pola pemanggilan `setVideoUrlLow('...')` / `setVideoHLS("...")`. */
function extractVideoUrl(script, functionName) {
    var m = script.match(new RegExp(functionName + "\\('([^']*)'\\)"));
    if (m && m[1]) return m[1];
    m = script.match(new RegExp(functionName + '\\("([^"]*)"\\)'));
    return (m && m[1]) ? m[1] : "";
}
...
9. **Pakai Mode Statis dulu, Mode Interaktif sebagai fallback.** `fetch()` + `RoCatDOM`
   adalah 95% kasus — murah, cepat, dan tidak bergantung engine WebView. Naikkan ke
   `RoCatPage` hanya saat HTML statis kosong (SPA, anti-bot, player JS-generated).
   Selalu panggil `RoCatPage.close()` di blok `finally` agar memori WebView bebas, dan
   cek `typeof RoCatPage !== "undefined"` karena engine tanpa `ScriptBrowserBridge`
   tidak meng-inject global ini.
...
| `RoCatPage` (dual-mode) | `scripting/api/.../ScriptBrowserBridge.kt` + `scripting/rhino/.../RoCatPageBridge.kt` + `app/.../scripting/HeadlessWebViewManager.kt` + `AppScriptBrowserBridge.kt` (wire via `ScriptEnvironment.browser`) |

> Skrip nyata yang memakai seluruh API ini:
>
> - **`scrape_anichin.js`** (Mode Statis murni) — pipeline `fetch` + Jsoup untuk
>   home/pencarian/detail + decode base64 mirror + pilih varian HLS master.
> - **`fixed_testscrape.js`** (Mode Hybrid, referensi Tahap 23) — alur
>   `onLaunch`/`doSearch`/`openDetail`/`openVideo` memakai `RoCat.render([...])`,
>   payload `RoCat.safeParseJson`, status `addAlert`, genre `addBadgeGroup`, debug
>   `addJsonLog`; ekstraksi html5player via `innerHtml` (CDATA), lalu fallback ke
>   `RoCatPage.open` + `waitForSelector` + `evaluate` + `close` ketika stream tidak
>   muncul di HTML statis (anti-bot / JS-generated player); pilih kualitas → kirim
>   `RoCatUI.addVideo(..., true, true)`.
>
> Pola umum: **statis dulu, interaktif sebagai fallback, `close()` di `finally`**.